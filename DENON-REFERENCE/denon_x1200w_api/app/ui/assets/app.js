(() => {
  ...
  async function routeFromHash() {
    ...
    const changed = state.route.view !== route.view;
    state.route = route;
    for (const el of document.querySelectorAll(".view")) el.hidden = true;
    const view = $(`view-${route.view === "info" ? "info" : "setup"}`);
    if (view) view.hidden = false;
    $("tab-setup")?.classList.toggle("active", route.view === "setup");
    $("tab-info")?.classList.toggle("active", route.view === "info");
    const pill = $("mode-pill");
    if (pill) pill.hidden = !state.connected;

    if (!state.connected) return;

    if (route.view === "info" && (changed || !state.infoLoaded)) {
  ...
  async function boot() {
    ...
    try {
      ...
      if (!data.reachable) {
        setStatus(
          `AVR not reachable. Check DENON_HOST in .env. ${data.probe?.error || ""}`,
          "err"
        );
        state.connected = false;
        footHost.textContent = "Host from .env";
        return;
      }
      ...
      $("main").hidden = false;
      await loadMenu();
      if (!location.hash || location.hash === "#") navigate("#/setup", { replace: true });
      else await routeFromHash();
    } catch (err) {
  ...
  async function openMenuNode(node) {
    if (state.route.view !== "setup") navigate("#/setup", { replace: true });
  ...
  window.addEventListener("hashchange", () => {
    routeFromHash().catch((err) => setStatus(err.message, "err"));
  });
